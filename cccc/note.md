...
### 001
* DP : 硬币外循环，钱数内循环。
* dp[j-coins[i]] + coins[i] == j
* 题目约束：我们说序列{A[1], A[2], ...}比{B[1], B[2], ...}“小”，是指存在 k >= 1 使得 A[i]=B[i] 对所有 i < k 成立，并且 A[k] < B[k]。

  1. 直觉来说，硬币数目越多，最前面硬币越小：dp[j-coins[i]] + coins[i] == j && num[j-coins[i]]+1 >= num[j]
  2. 取 >= 是因为先对硬币sort后从小到大遍历。>= 可以使得硬币序列前面的数尽量小，因为后面大了。
* 回溯：用traceback[j]记录到达j时用的是哪一个硬币
* 注意：coin要从小到大，否则大的硬币面值会被小的面值分解掉

### 002
* 主席树
* 对原来的数列[1..n]的每一个前缀[1..i]（1≤i≤n）建立一棵线段树，线段树的每一个节点存某个前缀[1..i]中属于区间[L..R]的数一共有多少个。
* 
#########主席树与O(n)的第k大数寻找算法？
...
* 并查集
* 将爱好合并，root记录user id
* 注意并查集合并的时候注意user id的合并

---
## Exp
1. 注意变量的名称，是否修改，作用域。
2. 对于区间划分型递归函数：确定区间开闭，验证len=0/1/2/3的情况。
---
## Need to learn
1. SPFA
2. Dilworth
